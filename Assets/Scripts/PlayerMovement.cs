...
public class PlayerMovement : MonoBehaviour
{
    ...
    public bool hasShield = false;      // Flag for shield status
    ...
    void ActivateShield()
    {
        hasShield = true; // set the flag to true
        shield.GetComponent<Shield>().ActivateShield(10f); // activate the shield for 10 seconds
    }

    void DeactivateShield()
    {
        hasShield = false; // set the flag to false
        shield.GetComponent<Shield>().DeactivateShield(); // deactivates the shield
    }
    ...
    {
        // Does the player have an active shield?
        if (HasShield())
        {
            DeactivateShield(); // Only deactivate the shield
            StartCoroutine(InvincibilityRoutine()); // short invincibility after shield is hit
            Debug.Log("Schild hat den Schaden abgefangen!");
            return; 
        }

        // If no shield and not invincible, take damage
        if (!isInvincible)
        {
            lives -= damage;
