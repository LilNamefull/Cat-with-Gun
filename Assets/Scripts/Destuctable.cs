...
public class Destuctable : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        // Did the Enemy got hit by a Bullet?
        Bullet bullet = collision.GetComponent<Bullet>();
        if (bullet != null)
        {
            //Is the Bullet from another Enemy?
            if (bullet.isEnemy)
            {
                 return; 
            ...
            else // Its the Bullet from the Player
            {
                TakeDamage(bullet.damage); // Enemy takes damage
    ...
    private void OnDestroy()
    {
        ...
        if (Level.instance != null && gameObject.scene.isLoaded)
        {
